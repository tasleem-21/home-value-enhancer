...
const AdminDashboard = () => {
  ...
  const getPropertyId = (property) =>
    property?.id ?? property?._id ?? property?.propertyId ?? property?.propertyID ?? property?.recommendationId;
  ...
  const loadRecommendations = async () => {
    ...
    try {
      ...
      const recentRaw = localStorage.getItem(RECENT_RECOMMENDATION_KEY);
      if (recentRaw) {
        try {
          const recentItem = JSON.parse(recentRaw);
          const recentId = getPropertyId(recentItem);
          const exists = normalizedItems.some((item) => {
            const itemId = getPropertyId(item);
            if (recentId !== undefined && recentId !== null && itemId !== undefined && itemId !== null) {
              return String(itemId) === String(recentId);
            }

            return (
              item?.title === recentItem?.title &&
              item?.category === recentItem?.category &&
              item?.costRange === recentItem?.costRange
            );
          });

          if (!exists) {
            normalizedItems.unshift(recentItem);
          }
        } catch {
          // Ignore malformed temporary data.
        }

        localStorage.removeItem(RECENT_RECOMMENDATION_KEY);
      }
  ...
  const loadUserSubmissions = () => {
    ...
    try {
      const byUserRaw = localStorage.getItem(USER_HISTORY_KEY);
      const byUser = byUserRaw ? JSON.parse(byUserRaw) : {};
      if (byUser && typeof byUser === 'object' && !Array.isArray(byUser)) {
        allSubmissions = Object.entries(byUser).flatMap(([userKey, list]) =>
          Array.isArray(list)
            ? list.map((item) => ({ ...item, userKey: item?.userKey || userKey }))
            : []
        );
      }
    } catch {
  ...
  const handleDeleteSubmission = (id) => {
    if (window.confirm('Are you sure you want to delete this user submission?')) {
      const updated = userSubmissions.filter((item) => String(item.id) !== String(id));
      ...
      try {
        ...
        const byUser = byUserRaw ? JSON.parse(byUserRaw) : {};
        if (byUser && typeof byUser === 'object' && !Array.isArray(byUser)) {
          const normalized = Object.entries(byUser).reduce((acc, [userKey, list]) => {
            const safeList = Array.isArray(list) ? list : [];
            acc[userKey] = safeList.filter((item) => String(item.id) !== String(id));
            return acc;
          }, {});
          localStorage.setItem(USER_HISTORY_KEY, JSON.stringify(normalized));
        }
      } catch {
        // Ignore malformed storage and keep UI updated.
      }
    }
  };
